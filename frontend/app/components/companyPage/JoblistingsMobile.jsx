    ...
    <div className="lg:hidden rounded-md bg-white shadow p-5 mt-10">
      ...
      <div className="flex flex-wrap gap-2">
        {jobListings.map((listing, index) => (
          <div
            key={index}
            className="card w-full md:max-w-xs md:mr-5 bg-slate-50 shadow-md mb-1"
          >
            <div className="card-body">
              ...
              <p className="text-sm hidden md:block">{listing.description}</p>
              <div>
                <p className="text-base">{listing.requirements}</p>
              </div>
            </div>
          </div>
        ))}
